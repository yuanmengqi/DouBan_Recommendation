...
class Aggregator(nn.Module):
    ...
    def forward(self, ego_embeddings, A_in):
        ...
        side_embeddings = torch._sparse_mm(A_in,ego_embeddings)                                             # (n_users + n_entities, embed_dim)

        if self.aggregator_type == 'gcn':
            # 2. Equation (6) 将中心节点表征和一跳邻域表征相加，再进行线性变换和非线性激活
            sum_embeddings = ego_embeddings+side_embeddings                                                   # (n_users + n_entities, embed_dim)
            embeddings = self.linear(sum_embeddings)                                                       # (n_users + n_entities, embed_dim)
            embeddings = self.activation(embeddings)                                                       # (n_users + n_entities, embed_dim)

        elif self.aggregator_type == 'graphsage':
            # 3. Equation (7) 将中心节点表征和一跳邻域表征拼接，再进行线性变换和非线性激活
            concat_embeddings = torch.cat([ego_embeddings, side_embeddings], dim=1)                         # (n_users + n_entities, embed_dim * 2)
            embeddings = self.linear(concat_embeddings)                                                       # (n_users + n_entities, embed_dim * 2)
            embeddings = self.activation(embeddings)                                                       # (n_users + n_entities, embed_dim)
...
class GNN_based(nn.Module):
    ...
    def calc_cf_embeddings(self):
        ...
        for idx, layer in enumerate(self.aggregator_layers):
            #使用聚合器逐层计算实体（包含用户）嵌入
            ego_embed = layer (ego_embed,self.A_in)#计算一跳邻域的表征
            #归一化嵌入的L2范数
            ego_embed = F.normalize(ego_embed,p=2,dim=1)
            all_embed.append(ego_embed)                                                                      # (n_users + n_entities, embed_dim)

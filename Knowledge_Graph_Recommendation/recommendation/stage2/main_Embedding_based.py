...
from parser1.parser_Embedding_based import *
...
def train(args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        time1 = time()
        total_loss = 0
        n_batch = data.n_cf_train // data.cf_batch_size + 1#//表示整数除法
        ...
        for iter in range(1, n_batch + 1):
            ...
            batch_loss = model(cf_batch_user, cf_batch_pos_item, cf_batch_neg_item, kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail, is_train=True) # 就是calc_loss？
    ...
    metrics_df.to_csv(args.save_dir + '', sep='\t', index=False)
